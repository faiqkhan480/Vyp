import 'package:vyv/utils/constants.dart';

class Api {
  static String middleWare = "/general";
  static String countries = "/country/all";
  static String categories = "/category/all";
  static String subCategories = "/subcategory/all";
  static String districts = "/district/country/";
  static String counties = "/county/country/";
  static String spot = "/spot";
  static String countryTab = "/spot/countryTab";
  static String districtTab = "/spot/districtTab";
  static String countyTab = "/spot/countyTab";
  static String district = "/spot/district/";
  static String county = "/spot/county/";
  static String spotDetail = "/spot/spot/";
  static String login = "/person/login/";
  static String register = "/person/register/";
  static String resetPass = "/person/update/password/";
  static String createFolder = "/favorite/create/";
  static String allFavorites = "/favorite/user/";
  static String folder = "/favorite/folder/";
  static String folderList = "/favorite/folder/page/user/";
  static String addFavorite = "/favorite/add/favorite/";
  static String delFavorite = "/favorite/delete/favorite/";
  static String nationality = "/general/nationality/all";
}